> **Active scope:** Governing architectural decision for the interval calibrator plugin contract (`IntervalCalibratorPlugin`) and calibrator registration strategy. Remains active as long as these contracts govern calibrator plugins; superseded when the calibrator plugin strategy is revised.
...
> **Terminology Note:** ADR-021 clarifies that "probabilistic regression" and "thresholded regression" are synonymous terms. "Probabilistic regression" is the user-facing term; "thresholded regression" is used for technical architecture discussions. See [ADR-021 Terminology section](./ADR-021-calibrated-interval-semantics.md#terminology-probabilistic-regression-vs-thresholded-regression) for full context.
