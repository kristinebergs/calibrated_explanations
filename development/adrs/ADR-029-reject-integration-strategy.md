> **Active scope:** Governing architectural decision for the `RejectPolicy`/`RejectOrchestrator`/`RejectResult` reject framework contract. Post-v1.0 items (strategy lifecycle hooks, strict migration) are tracked in §Open Items; remains active until those items are resolved or formally deferred.
...
- **Regression constraint:** The reject framework supports regression **only when a
  decision threshold is provided** to `initialize_reject_learner(threshold=t)`. The
  framework converts regression into threshold-binarized conformal classification
  (`P(y ≤ threshold)`); this is distinct from conformal prediction intervals, which
  remain a separate CE capability. Omitting `threshold` for a regression explainer
