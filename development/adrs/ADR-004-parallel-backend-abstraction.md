> **Active scope:** Governing architectural decision for the `ParallelConfig`/`strategy` parallel-execution contract and backend selection policy. The `strategy="auto"` deprecation (v0.11.4) and removal (v1.0.0) are implementation milestones within this ADR's lifecycle.
