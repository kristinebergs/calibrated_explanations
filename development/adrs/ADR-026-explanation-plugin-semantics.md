> **Active scope:** Governing architectural decision for the semantic contract of explanation plugin outputs (rule structure, feature weights, prediction fields, `conjunctive_rules`). Remains active as long as the plugin output contract governs explanation assembly; superseded when the semantic contract is revised.
...
* As of v0.11.3, guarded execution **is** an explanation-plugin concern. The `guarded`
  ...
  guarded execution is requested; a `ValidationError` is raised if no capable plugin is found.
