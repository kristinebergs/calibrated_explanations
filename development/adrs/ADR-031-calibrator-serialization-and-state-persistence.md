> **Active scope:** Governing architectural decision for the calibrator save/load serialization contract and the `joblib`/`pickle` persistence boundary. Remains active as long as this contract governs calibrator state persistence; superseded when the persistence strategy is revised.
