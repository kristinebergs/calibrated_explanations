> **Active scope:** Governing architectural decision for backward-compatibility guarantees on the public `WrapCalibratedExplainer` and `CalibratedExplainer` API through v1.0.0. Moves to `finished-work` after v1.0.0 GA when the stability contract transitions to a post-GA maintenance policy.
...
and conjunction tooling without warning.【F:docs/getting_started.md†L5-L413】【F:README.md†L150-L198】【F:development/finished-work/legacy_user_api_contract.md†L9-L93】

## Decision

Adopt a layered guardrail strategy that freezes the documented legacy API:

1. **Canonical contract.** The new `development/finished-work/legacy_user_api_contract.md`
   ...
   legacy-support decisions and must accompany any future user-facing change.【F:development/finished-work/legacy_user_api_contract.md†L1-L103】
2. **Signature regression tests.** Add unit tests that inspect the signatures of
   ...
   `.explore_alternatives` so accidental removals fail fast.【F:development/finished-work/legacy_user_api_contract.md†L15-L93】
