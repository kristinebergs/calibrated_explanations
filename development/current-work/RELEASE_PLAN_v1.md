...
## Current released version: v1.0.0rc2

> Status: v1.0.0rc2 shipped on 2026-07-20. Release artifacts were verified on PyPI by `make release-postcommit`.
...
- **Current released version:** v1.0.0rc2
- **Active detailed milestone:** v1.0.0 (`development/current-work/v1.0.0_plan.md`) — no longer blocked; `v1.0.0rc2` shipped 2026-07-20 (tag `v1.0.0rc2`, commit `7a7e9ff3`) and was verified on PyPI by `make release-postcommit`.
...
**ADR-012 - Documentation & Gallery Build Policy:** Accepted; gallery-tooling decision closed (nbconvert, 2026-06-02). Re-evidenced 2026-06-11: notebook execution exists (advisory driver with timeouts; `nbsphinx_execute="always"` on non-RTD builds). Gap 1 closed (v0.11.4): docs HTML/linkcheck CI jobs wired (since v0.11.6 Task 60: strict HTML build in `ci.yml` `docs-build`, linkcheck and notebook execution in weekly `scheduled.yml`). Gap 2 (per-example runtime ceiling enforcement) remains advisory-only; blocking enforcement is a release-branch obligation. Target: v1.0.0-rc.
...
2. Reconfirm the ADR-020 legacy wrap contract established during the v0.7.0
   transition, account for intentional user-visible migration changes since
   v0.6.1, verify the exception taxonomy, and confirm that v0.11.6 to RC adds no
   unintended public-surface changes; update README and CHANGELOG with a
   release-candidate compatibility note.?F:src/calibrated_explanations/core/wrap_explainer.py?L260-L471??F:src/calibrated_explanations/core/exceptions.py?L1-L63?
