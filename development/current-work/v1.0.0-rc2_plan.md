...
> **Development version:** `1.0.0-rc-dev` (corrected 2026-07-20 from the
> stale GA-track `1.0.0-dev` left by rc1's postcommit bump, which predated
> this plan's insertion; `1.0.0-rc-dev` restores the same rc-family lineage
> used throughout the rc1 cycle so the release-preflight tooling's
> plan-discovery and version-lineage checks resolve to this plan. The
> release preflight tooling produces the `1.0.0rc2` candidate version — do
> not hand-pin it.)
...
- [x] `plotting.py` implements `_PlotDispatchOutcome`, `_is_third_party_plot_style`,
      `_reject_conflicting_plot_selection`, `_normalize_third_party_transport`,
      `_third_party_plot_options`, `_dispatch_third_party_plot`,
      `_dispatch_explicit_instance_plot_style`,
      `_dispatch_explicit_global_plot_style` (commit `9e1fa1e5`, corrects `8459df1c`).
- [x] `FactualExplanation.plot` and `AlternativeExplanation.plot` intercept
      third-party styles as their first action.
- [x] `PluginManager.resolve_plot_plugin_strict` added: exact identifier
      resolution; deny/unregistered/incomplete raise `ConfigurationError`;
      renderer-override must resolve or raise; returns a trust flag.
- [x] `PlotRenderContext.runtime` added (default `{}`, top-level read-only,
      dropped on pickle).
- [x] Independent review correction pass applied (`9e1fa1e5`) covering
      configured-preference styles, `style_override`-only selection, and
      renderer-override trust recomputation.
...
- [x] `tests/unit/test_plot_third_party_dispatch.py` — 25 new tests
      (factual/alternative/global/dashboard forwarding, strict resolution,
      trust gating, transport).
- [x] `tests/plugins/test_protocols.py` — 5 new `PlotRenderContext`
      construction/read-only/pickle tests.
- [x] Existing tests adapted, not weakened: `test_plot_default_promotion.py`
      shims stub the strict resolver and use a renderer-native artifact
      shape; `test_plotting_coverage.py` renderer-override fallback test
      moved to the configured-preference path.
...
- [x] CE wheel (no `viz` extra) + `ce-synthetic-viz` wheel verified in a
      clean Python 3.11 env without Matplotlib.
- [x] All four synthetic styles pass with complete options, runtime
      dashboard access, `None`-result handling, and unchanged callable
      identities.
- [x] 20/20 checks recorded in the evidence report (2026-07-19).
...
- [x] Temporary branch `tmp/no-bridge-proof` removes
      `install_ce_plot_bridges()`/`_ce_compat` and ports dashboard
      precomputation onto `context.runtime`.
- [x] All six bridge-covered styles pass through public CE APIs from
      installed wheels; CE callable identities unchanged throughout.
- [x] 19/19 checks recorded in the evidence report (2026-07-20).
...
- [x] `docs/contributor/plugin-contract.md` "Third-party style dispatch
      contract (v1.0.0rc2)" section added (dispatch timing, built-in
      vocabulary, strict resolution, transport semantics,
      `context.options`, reserved global `payload`, runtime semantics +
      serialization limits, trust implications, artifact validation,
      result propagation, no-monkey-patching rule, minimal factual and
      global/dashboard examples).

## 6) Release preparation

- [x] Merge `feat/plot-plugin-dispatch-rc2` to `main` after review.
- [x] Run `make release-preflight VERSION=1.0.0rc2` and `make
      release-finalize` to produce the `1.0.0rc2` candidate version (2026-07-20,
      both exit 0). An explicit `VERSION=` override was required: default plan
      discovery matched the stale dev-track lineage (see the development-version
      note above) to `v1.0.0_plan.md` instead of this plan. Fixed by correcting
      `pyproject.toml`/`__init__.py`'s dev version from `1.0.0-dev` to
      `1.0.0-rc-dev`, and by restructuring this plan's task headers/checklists
      to the schema `scripts/local_checks.py` requires (`## N)` headings +
      `### N.M Verification checklist` subsections) — the plan previously used
      `### N)` headings with one shared checklist, which the readiness guard
      could not parse.
...
- [x] `make local-checks-pr` on the candidate commit — covered by the
      `--profile release` run inside `make release-preflight` (2026-07-20,
      exit 0), which is a strict superset of `--profile pr`.
- [x] Strict docs build / link check — `make release-preflight`'s release
      profile ran both "Strict docs build" and "Docs linkcheck" (2026-07-20,
      exit 0).
- [x] Public API snapshot generated — `scripts/quality/snapshot_public_api.py`
      ran as part of the release profile (2026-07-20); latest snapshot at
      `scripts/tests/benchmarks/api_public_20260720_182626.txt`.
- [ ] Public API snapshot reviewed for `PlotRenderContext.runtime` and
      `PluginManager.resolve_plot_plugin_strict` through the governed human
      review process (generation alone does not close this item).
