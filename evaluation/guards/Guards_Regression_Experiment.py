...
resultfile = "evaluation/results_guards_regression.pkl"
...
setups = ["lime", "shap", "ce", "pce", "ce_guarded", "ce_unguarded", "pce_guarded", "pce_unguarded"]
...
for dataSet, filename in zip(datasets, filenames):
    ...
    if (
        no_of_instances < 800
    ):  # skip datasets with less than 800 instances, as 100 + 200 + 500 = 800
        continue
    ...
    for c1, model, alg, X in model_struct:
        ...
        X_train, X_cal, y_train, y_cal = train_test_split(
            X_trainCal, y_trainCal, test_size=cal_size, random_state=42
        )
        ...
        while i < num_rep:
            ...
            ce_guarded = CalibratedExplainer(
                model, X_cal, y_cal, mode="regression", random_state=i, guard="conformal_regions"
            )
            ce_unguarded = CalibratedExplainer(
                model, X_cal, y_cal, mode="regression", random_state=i, fast=True
            )
            ...
            for norm in normalizations:
                ...
                ct = time.time() - tic
                stab_timer["ce_guarded" + norm].append(ct / len(X_test))
                print(f"{letter}ceg{ct/len(X_test):.4f}", end=" ", flush=True)
                stability["ce_guarded" + norm].append([f.feature_weights for f in explanations])

                tic = time.time()
                explanations = ce_unguarded.explain_fast(X_test)
                ct = time.time() - tic
                stab_timer["ce_unguarded" + norm].append(ct / len(X_test))
                print(f"{letter}ceu{ct/len(X_test):.4f}", end=" ", flush=True)
                stability["ce_unguarded" + norm].append([f.feature_weights for f in explanations])

                tic = time.time()
                explanations = ce_guarded.explain_factual(X_test, threshold=0.5)
                ct = time.time() - tic
                stab_timer["pce_guarded" + norm].append(ct / len(X_test))
                print(f"{letter}pceg{ct/len(X_test):.4f}", end=" ", flush=True)
                stability["pce_guarded" + norm].append([f.feature_weights for f in explanations])

                tic = time.time()
                explanations = ce_unguarded.explain_fast(X_test, threshold=0.5)
                ct = time.time() - tic
                stab_timer["pce_unguarded" + norm].append(ct / len(X_test))
                print(f"{letter}pceu{ct/len(X_test):.4f}", end="\n", flush=True)
                stability["pce_unguarded" + norm].append([f.feature_weights for f in explanations])
            # print(f'',end='\n', flush=True)
            i += 1
        ...
        while i < num_rep:
            ...
            X_train, X_cal, y_train, y_cal = train_test_split(
                X_trainCal, y_trainCal, test_size=cal_size, random_state=i
            )
            # X_train = X_train[:1000]
            # y_train = y_train[:1000]
            ...
            for norm in normalizations:
                ...
                explanations = ce_guarded.explain_factual(X_test)
                ct = time.time() - tic
                rob_timer["ce_guarded" + norm].append(ct / len(X_test))
                print(f"{letter}ceg{ct/len(X_test):.4f}", end=" ", flush=True)
                robustness["ce_guarded" + norm].append([f.feature_weights for f in explanations])

                tic = time.time()
                explanations = ce_unguarded.explain_fast(X_test)
                ct = time.time() - tic
                rob_timer["ce_unguarded" + norm].append(ct / len(X_test))
                print(f"{letter}ceu{ct/len(X_test):.4f}", end=" ", flush=True)
                robustness["ce_unguarded" + norm].append([f.feature_weights for f in explanations])

                tic = time.time()
                explanations = ce_guarded.explain_factual(X_test, threshold=0.5)
                ct = time.time() - tic
                rob_timer["pce_guarded" + norm].append(ct / len(X_test))
                print(f"{letter}pceg{ct/len(X_test):.4f}", end=" ", flush=True)
                robustness["pce_guarded" + norm].append([f.feature_weights for f in explanations])

                tic = time.time()
                explanations = ce_unguarded.explain_fast(X_test, threshold=0.5)
                ct = time.time() - tic
                rob_timer["pce_unguarded" + norm].append(ct / len(X_test))
                print(f"{letter}pceu{ct/len(X_test):.4f}", end="\n", flush=True)
                robustness["pce_unguarded" + norm].append([f.feature_weights for f in explanations])
            i += 1
            # except Exception as e: # pylint: disable=broad-exception-caught
            #     warnings.warn(f'Error: {e}')
            # print('')
