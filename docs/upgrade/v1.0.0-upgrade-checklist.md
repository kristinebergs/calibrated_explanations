...
Use this checklist when migrating from v0.6.1 or any later pre-v1 release to
v1.0.0. Work through each topic in order; most items are verifiable with a
one-line command. If you are already on v0.11.x, you may skip the earlier
v0.7-v0.10 migration rows that do not apply to your code.

## Scope and compatibility meaning

This is cumulative migration guidance for the documented public workflows
protected by ADR-020. It is not a promise that every historical keyword or
removed alias remains source-compatible, and it is not a namespace diff for
transient internal APIs.

Three comparison anchors define the v1 compatibility review:

- **v0.6.1** is the oldest migration anchor covered by the v1 roadmap.
- The **v0.7.0-era ADR-020 contract** is the protected legacy-workflow anchor;
  it was recorded after the v0.7 signature normalization.
- **v0.11.6** is the RC-freeze anchor; `1.0.0rc1` must introduce no unintended
  public-surface changes relative to it.

The removed-symbol history in
[`docs/migration/deprecations.md`](../migration/deprecations.md) is authoritative.
The [v0.11.4](v0.11.4-upgrade-checklist.md) and
[v0.11.6](v0.11.6-upgrade-checklist.md) checklists provide additional detail for
their fail-fast validation and configuration changes.
...
All 26 recognised configuration/environment indicators are listed below. Keys
not set default to their pyproject.toml value or the library default.
...
## 3. API changes since v0.6.1

### Renamed in v0.7.0

The v0.7.0 nomenclature pass renamed public parameters before ADR-020 captured
the protected legacy workflow. Positional calls are unaffected; keyword calls
must use the v1 names.

| v0.6.1 keyword | Canonical keyword | Affected public surfaces |
|---|---|---|
| `X_proper_train` | `x_proper_train` | `WrapCalibratedExplainer.fit(...)` |
| `X_calibration` | `x_calibration` | `WrapCalibratedExplainer.calibrate(...)` |
| `X_test` | `x` | Wrapper and `CalibratedExplainer` explanation, prediction, reject, and plot methods that accepted test data |
| `y_test` | `y` | Wrapper and `CalibratedExplainer` plot methods |

The historical wrapper methods listed in this table may themselves have a later
replacement. Continue through the v0.11 removal tables below before updating a
call site.

### Changed in v0.10.0

`CalibratedExplainer.explain(...)` was internalized. Select the intended public
operation explicitly:

- `explain_factual(...)` for factual explanations;
- `explore_alternatives(...)` for alternative explanations; or
- `explain_fast(...)` for the opt-in FAST path.

The package-root public facade was also restricted. Import public types not
listed at the package root from their documented submodules; the concrete
top-level removals and replacements are listed in the v0.11.0 table below.
...
### Exception import compatibility

The original v0.6.1 exception classes remain available through
`calibrated_explanations.core.exceptions`. The canonical v1 import path is
...
The original classes are preserved rather than renamed or removed. The
`core.exceptions` module remains a compatibility re-export, but new code should
use the canonical `utils.exceptions` path. Additional classes on the v1 surface
are additive and do not break handlers written against `CalibratedError`.
...
|---|---|
| Declare `data_modalities` in every entry-point plugin's `plugin_meta` | Add `"data_modalities": ("tabular",)` (or `"vision"`, etc.) to your `plugin_meta` dict. Plugins missing this key are excluded from discovery with a `UserWarning` as of v0.11.3. |

Import: `from calibrated_explanations.explanations.guarded_options import GuardedOptions`

The `guarded=True` boolean kwarg was removed in v0.11.5.
...
kwarg was **deprecated as of v0.11.3** and removed in v0.11.5.
...
`EXPLAIN_NON_REJECTS`, and `SKIP_ON_REJECT` were removed
in v0.11.3 and raise `ValueError` / `AttributeError` at runtime.

Other v0.11.5 removals include `CalibratedExplanations.legacy_payload(exp)`;
use `to_json()` for serialization workflows.
...
## 10. Research reproduction assets

Evaluation code, datasets, notebooks, study environments, and result artefacts
are maintained in
[`kristinebergs/calibrated-explanations-studies`](https://github.com/kristinebergs/calibrated-explanations-studies).
Core installations no longer include the full evaluation suite or an `eval`
extra. To reproduce a paper, follow the relevant study README and install the
CE version and dependencies specified there; the latest CE release is not
guaranteed to reproduce historical results exactly.
