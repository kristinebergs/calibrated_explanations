...
- Legacy plot code follows the repo-wide fallback visibility policy: any runtime fallback or simplification MUST emit an `INFO` log and a `warnings.warn(..., UserWarning)` (see `.github/copilot-instructions.md` and `CONTRIBUTOR_INSTRUCTIONS.md` §5). Tests that rely on fallbacks should assert the warning via `pytest.warns(UserWarning)`.
