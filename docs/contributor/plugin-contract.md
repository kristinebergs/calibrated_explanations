...
1. **Explainer parameters** (highest priority): Direct kwargs to `WrapCalibratedExplainer.calibrate(...)` (e.g., `factual_plugin`, `interval_plugin`, `plot_style`).
...
Older integrations sometimes reached into the underlying calibrated explainer internals or
...
### Method A: Via WrapCalibratedExplainer calibration parameters

Pass the desired plugin identifier (or style for plot plugins) when calibrating the wrapper:
...
chain = explainer.explainer.plugin_manager.plot_style_chain
...
The underlying calibrated explainer stores the plugin identifiers and uses them during explanation generation.
